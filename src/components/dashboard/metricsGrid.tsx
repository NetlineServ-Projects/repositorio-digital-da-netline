...
export default function MetricsGrid({
  ...
  return (
    <div className={`grid grid-cols-1 md:grid-cols-2 ${ehAdmin ? "lg:grid-cols-5" : "lg:grid-cols-4"} gap-6`}>
      ...
      {ehAdmin && (
        <MetricCard titulo="Pendentes de Aprovação" valor={pendentesAprovacao ?? 0} icone={<IconRelogio className="w-6 h-6" />} corBorda="border-t-amber-500" corIcone="text-amber-600" />
      )}
