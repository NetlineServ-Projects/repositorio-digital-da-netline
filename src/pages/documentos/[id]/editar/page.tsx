  ...
  if (loading) {
    return (
      <div className="flex items-center justify-center p-12">
        <p className="text-slate-500 font-medium animate-pulse text-sm">{t("documentos.detalhes.carregando")}</p>
      </div>
    );
  }
  ...
  if (erro || !documento) {
    return (
      <div className="bg-white p-6 rounded-xl border border-slate-100 shadow-sm text-center space-y-3 max-w-lg">
        <p className="text-red-600 text-sm font-medium">{erro || t("documentos.detalhes.naoEncontrado")}</p>
  ...
  return (
    <div className="space-y-6 max-w-xl">
      ...
      <div className="bg-white rounded-xl border border-slate-100 shadow-sm p-6 space-y-5">
        <h1 className="font-bold text-slate-800 text-lg">{t("documentos.editar.titulo")}</h1>

        <div className="space-y-1.5">
          <label className="text-xs font-semibold text-slate-500 uppercase">{t("documentos.editar.tituloLabel")}</label>
          <input
            type="text"
            value={titulo}
        ...
        <div className="space-y-1.5">
          <label className="text-xs font-semibold text-slate-500 uppercase">{t("documentos.editar.descricaoLabel")}</label>
          <textarea
            value={descricao}
        ...
        <div className="space-y-1.5">
          <label className="text-xs font-semibold text-slate-500 uppercase">{t("documentos.editar.categoriaLabel")}</label>
          <select
            value={categoriaId}
        ...
        <div className="flex gap-3 pt-2">
          ...
          >
            {salvando ? t("documentos.aGuardar") : t("documentos.guardar")}
          </button>
        </div>
      </div>
    </div>
